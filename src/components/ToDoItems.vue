...
<template>
    <div class="space-y-2">
        <div v-for="todo in todoStore.todos" :key="todo.id" class="bg-gray-300 rounded-sm">
            <div class="flex items-center px-4 py-3 border-b border-gray-400 last:border-b-0">
                ...
                <div class="w-full">
                    <!-- @blur: só atualiza quando o usuário sai do input. -->
                    <input 
                        type="text" 
                        ...
                        @blur="todoStore.updateTitle(todo.id, todo.title)"
                    >
                </div>
